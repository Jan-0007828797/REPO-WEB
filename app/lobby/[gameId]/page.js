...
export default function Lobby(){
  ...
  useEffect(()=>{ (async ()=>{ if(!joinUrl) return; setQr(await QRCode.toDataURL(joinUrl,{margin:1,width:260})); })(); },[joinUrl]);
  useEffect(()=>{
    const s=getSocket();
  ...
  const canStart = isGM && cfg && players.length===cfg.maxPlayers;
  return (
